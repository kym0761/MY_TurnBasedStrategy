...
플러그인이나 외부 에셋은 사용하지 않고 자신의 C++ 코드와 언리얼 엔진에서 제공하는 기능으로 구현한 프로젝트다.  
...
두 유닛의 애니메이션이 끝난 것을 확인한 뒤에, 다음 공격 Order대로 애니메이션을 재생하여 계산된 결과에 맞춰 공격을 할 것이다.  
공격 계산 결과대로 100% 데미지를 받는 것은 절대 아니며, Crit 확률에 따라 데미지를 2배로 받을 수 있거나, 회피 확률에 따라 공격을 무시하는 회피 애니메이션이 재생되어 데미지를 받지도 않을 수도 있다.


만약 둘 중 어떤 유닛이 공격 순서대로 전투 결과를 재생하는 중에 HP가 0이 되어 죽게된다면, 이후 전투 결과는 재생되지 않고 전투가 끝난다.  
죽은 유닛에게는 죽고나서 사라지는 유예기간을 잠시 가지며, 유예기간이 끝나 죽는다면 GridManager에서 죽은 유닛이 점유하고 있는 Grid에서 벗어나고, 죽은 유닛이 사라지면서 플레이어가 그 후 행동을 할 수 있게 된다.
...
다른 예시로, 적 유닛이 내 유닛을 공격할 때, 적 유닛이 죽는다면 적 유닛의 행동에 대한 OnActionEnd가 동작하여 GameMode가 유닛의 숫자를 파악한다. 이때도 적 유닛이나 플레이어의 유닛의 숫자가 0이 된 상태라면 게임의 결과가 나온다.  
